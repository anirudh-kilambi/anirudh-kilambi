...
T_model_K = T_model + 273
convection = U * A * (T_a - T_model_K)
radiation = epsilon * sigma * A * (T_inf**4 - T_model_K**4)
loss = convection + radiation


plt.figure(1)
plt.title("Modelled and Measured Temperature vs Time")
...
plt.title("Convective, Radiative, and Total Loss")
plt.plot(tm, convection, "r--", label="Convection")
plt.plot(tm, radiation, "g--", label="Radiation")
plt.plot(tm, loss, "b--", label="Loss")
